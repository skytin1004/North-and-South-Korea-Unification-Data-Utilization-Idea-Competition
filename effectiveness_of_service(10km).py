...
# 각 특산물 위치에서 10km 이내에 철도역이 있는지 확인
local_products['특산물 위치 10km 이내의 철도역'] = False
for idx, product in local_products.iterrows():
    for i, station in railway_stations_df.iterrows():
        ...
        if distance <= 10:
            local_products.at[idx, '특산물 위치 10km 이내의 철도역'] = True
            break

# 철도역이 10km이내에 있는 특산물의 비율 계산
percentage_with_selected_station = (local_products['특산물 위치 10km 이내의 철도역'].sum() / len(local_products)) * 100

percentage_with_selected_station

# 데이터 구성
labels_selected = ['10km이내에 철도역이 있음', '10km이내에 철도역이 없음']
sizes_selected = [local_products['특산물 위치 10km 이내의 철도역'].sum(),
                  len(local_products) - local_products['특산물 위치 10km 이내의 철도역'].sum()]
...
plt.axis('equal')
plt.title('북한 특산물 주변 철도역 분석(10km)')
plt.show()
